...
const Button = ({ children, className = '', variant = 'default', as: Component = 'button', ...props }) => {
...
export default function DymokApp() {
  ...
  const getTotalPrice = useCallback(() => {
    ...
    return promoCode === 'DISCOUNT10' ? subtotal * 0.9 : subtotal;
  }, [cart, promoCode]);
  ...
  const generateOrderNumber = useCallback(() => {
    ...
    return `ORD-${datePart}-${Math.floor(1000 + Math.random() * 9000)}`;
  }, []);
  ...
  const removeFromCart = useCallback((productId, flavor) => {
    setCart(prevCart => prevCart.filter(
      item => !(item.id === productId && item.selectedFlavor === flavor)
    ));
  }, []);

  const updateQuantity = useCallback((productId, flavor, newQuantity) => {
    if (newQuantity < 1) return removeFromCart(productId, flavor);
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 pb-20">
        {filteredProducts.map(product => (
          <Card key={product.id} className="hover:shadow-lg hover:shadow-green-500/10 transition-all duration-300">
            <CardContent>
              ...
              <div className="p-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    size="sm"
                    className="bg-green-600 hover:bg-green-700 text-xs px-3 py-1 rounded-lg"
  ...
  const renderCheckout = () => (
    <div className="pb-20">
      <div className="flex items-center mb-6">
        <Button variant="ghost" size="icon" className="mr-2" onClick={() => setShowCheckout(false)}>
          <ArrowLeft size={20} />
        </Button>
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-2">
                {['square', 'city', 'outside'].map(area => (
                  <label key={area} className="flex items-center gap-3 p-3 bg-gray-700/30 rounded-lg border border-gray-600">
                    <input
                      type="radio"
                      name="deliveryArea"
                    ...
                    <div className="flex-1">
                      <p>
                        {area === 'square' ? 'В квадрате Саина – Рыскулова – Восточка – Аль-Фараби' : 
                         area === 'city' ? 'В пределах города (за пределами квадрата)' : 'За город'}
                      </p>
                      <p className="text-sm text-gray-400">
                        {area === 'square' ? '1500₸' : 
                         area === 'city' ? '2500₸' : 'Индивидуальный тариф'}
                      </p>
                    </div>
                  </label>
                ))}
        ...
        {paymentConfirmed ? renderReceipt() : (
          <div className="space-y-4">
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-gray-100 px-4 py-3">
      ...
      {!showCheckout && (
        <div className="relative mb-6">
          <Input
            ...
            className="w-full py-5 px-4 focus:ring-2 focus:ring-green-500 focus:border-transparent"
          />
        </div>
      )}
      ...
      {!showCheckout && (
        <div className="fixed bottom-6 right-6 z-40">
          <Button
            className="relative rounded-full p-4 shadow-xl transition-all duration-300 group bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600"
